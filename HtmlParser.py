...
class HtmlParser(object):
    ...
    def _get_new_urls(self, page_url, soup):
        ...
        comments_count = unicode(soup.find(id='total-comments').string).encode('utf-8')
    ...
    # 这个是解析图书评论的
    def _get_new_comments(self,page_url,soup):
        data = {}
        try:
            data['book_id'] = ''
            if page_url.split('/')[-2] == 'comments' :
                data['book_id'] = page_url.split('/')[-3]
            else:
                data['book_id'] = page_url.split('/')[-2]
            data['list'] = []
            comments = soup.find(id='comments', class_='comment-list hot ').find_all('li')
            for child in comments:
                child_list = {}
                # user_id
                if child.find('a', href=re.compile(r'people')):
                ...
                # user_img
                if child.find('a', href=re.compile(r'people')).find('img')['src']:
                    child_list['user_img'] =  child.find('a', href=re.compile(r'people')).find('img')['src']
                else:
                    child_list['user_img'] = ''

                # valid_count
                if child.find('span',class_='vote-count'):
                    child_list['valid_count'] = child.find('span',class_='vote-count').get_text()
                else:
                    child_list['valid_count'] = ''

                # user_name
                if child.find('a', href=re.compile(r'people')):
                    child_list['user_name'] = child.find('a', href=re.compile(r'people'))['title']
                else:
                    child_list['user_name'] = ''
                ...
                if child.find(text=re.compile(r'\d*-\d*-\d')):
                    child_list['time'] = unicode(child.find(text=re.compile(r'\d*-\d*-\d')))
                else:
                    child_list['time'] = ''
